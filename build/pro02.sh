...
#Andrew@maxinvoke.com
#2018 
########################################################################################################


#Bash script to add package repository 
# Targeting  Microk8s Docker Chef UI 
...
install_common( )
{
    #due to interminitent error messages each package is installed on its only line verses compacting. 
	sudo apt-get install -yq default-jre
	sudo apt-get install -yq docker-ce
	sudo snap install microk8s --classic --beta
	sudo snap alias microk8s.kubectl kubectl
	sudo apt-get install -yq jenkins
	sudo apt-get install -yq figlet 
	sudo apt-get update -yq
